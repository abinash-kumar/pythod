...
from vyomab.utils import get_ip_address
from vyomab.tasks import update_data_in_vyom
from vyomab.settings import (
    TRACK_AJAX_REQUESTS,
...
class VisitorTrackingMiddleware(MiddlewareMixin):
    ...
    def _get_data_to_update_in_vyom(self, user, request, visit_time):
        # A Visitor row is unique by session_key
        session_key = request.session.session_key
        ip_address = get_ip_address(request)
        ...
        user_agent = request.META.get('HTTP_USER_AGENT', None)
        if user_agent:
            user_agent = smart_text(user_agent, encoding='latin-1', errors='ignore')
        data = {
            ...
            'request_isajax': request.is_ajax(),
        }
        return data
    ...
    def process_response(self, request, response):
        ...
        now = timezone.now()
        # update data in vyom
        data = self._get_data_to_update_in_vyom(user, request, now)
        # import ipdb; ipdb.set_trace()
        # session_data = self._serialize_session_object(request.session)
        update_data_in_vyom.delay(data)
